...
def render_structure(
    id: str, taskid: str, elements: list[FormElement], instructions: str
):
    with me.box(
        style=me.Style(
            padding=me.Padding.all(BOX_PADDING),
...
async def cancel_form(e: me.ClickEvent):
    ...
    key_parts = e.key.split('_')
    form_message_id, taskid = key_parts[0], key_parts[1]
    app_state.form_responses[messageid] = form_message_id
...
async def send_response(
    id: str, taskid: str, state: State, app_state: AppState
):
    messageid = str(uuid.uuid4())
    app_state.background_tasks[messageid] = ''
    app_state.form_responses[messageid] = id
    form = FormState(**json.loads(state.forms[id]))
...
async def submit_form(e: me.ClickEvent):
    try:
        state = me.state(State)
        key_parts = e.key.split('_')
        id, taskid = key_parts[0], key_parts[1]
        form = FormState(**json.loads(state.forms[id]))
        ...
        await send_response(id, taskid, state, app_state)
    except Exception as e:
